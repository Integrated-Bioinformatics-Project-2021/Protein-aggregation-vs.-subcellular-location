...
significance_subcellular_nbAPR <- function(data) {
	sig_subc_APR_prot <- kruskal.test(data$Nb_APRs ~ Subcellular_location, data = data)
...
get_krustal_test_results_nbAPR <- function(data) {
	sig_subc <- kruskal.test(Nb_APRs ~ Subcellular_location, data =  data)
	return (sig_subc)
}
...
get_wilcox_test_table_nbAPR <- function(data) {
	wilcox_sig_subc_com_prot<-pairwise.wilcox.test(data$Nb_APRs, data$Subcellular_location,
                     p.adjust.method = "BH")
	table_sig_subc_com_prot<-wilcox_sig_subc_com_prot$p.value
	heatmap(as.matrix(table_sig_subc_com_prot), scale="column", Colv = NA, Rowv = NA)
...
plot_stat_significance_nbAPR <- function(data){
  ggplot(data, aes(x=Subcellular_location, y = Nb_APRs, fill = Secretory),
         color = "supp", palette = "jco",
         add = "jitter") +
    rotate_x_text(angle = 45)+
    geom_violin() + 
    geom_hline(yintercept = mean(data$Nb_APRs), linetype = 2)+
    stat_summary(fun=mean, geom="point", shape=20, size=5, color="red", fill="red") +
