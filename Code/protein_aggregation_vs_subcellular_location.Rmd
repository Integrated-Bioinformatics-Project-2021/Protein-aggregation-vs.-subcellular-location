...
significance_tango_scores_complete_proteins_joined_secreted()
```
...
significance_tango_scores_APR_proteins_joined_secreted()
```

Is this significantly different from the analysis without proteins that don't have an APR region?

```{r}
# source("statistical_analysis.R")

# F_test(unlist(tango_scores_complete_protein, recursive = TRUE, use.names = TRUE), unlist(tango_scores_APR_protein, recursive = TRUE, use.names = TRUE))
# TODO
```
...
significance_tango_scores_max_joined_secreted()
```
...
##INTERESTED GROUPINGS Postive charged (positive) --> K <br /> Negative charged (negative) --> R <br /> Uncharged (uncharged) --> D <br /> Special cases (special) --> E <br /> Hydrophobic (hydrophobic) --> S <br /> PERCENTAGE OF INTERESTED GROUPS FOR ALL THE GK SIDES

```{r}
GK_analysis_GK_groupings = analyse_gate_keeper_groupings(GK_analysis$cts_gk_groups_side)
side_string = "GK"
```

```{r}
pie_plot_percentage_of_specific_residue(GK_analysis_GK_groupings$positive, "positive charged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_groupings$negative, "negative charged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_groupings$uncharged, "uncharged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_groupings$special, "special cases", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_groupings$hydrophobic, "hydrophobic", side_string)
```

PERCENTAGE OF INTERESTED RESIDUES FOR ALL OF THE GK + FL SIDES

```{r}
GK_analysis_GK_and_FL_groupings = analyse_gate_keeper_groupings(GK_analysis$cts_gk_fl_groups_side)
side_string = "GK + FL"
```

```{r}
pie_plot_percentage_of_specific_residue(GK_analysis_GK_and_FL_groupings$positive, "positive charged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_and_FL_groupings$negative, "negative charged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_and_FL_groupings$uncharged, "uncharged", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_and_FL_groupings$special, "special cases", side_string)
pie_plot_percentage_of_specific_residue(GK_analysis_GK_and_FL_groupings$hydrophobic, "hydrophobic", side_string)
```
...
for (i in 1:length(search_terms)) {
  ...
  counts_side_gk_fl_groups <- analyse_sides(counts$cts_gk_groups_side)
  ...
  pie_plot_sides(counts_side_gk$pos_ct, "negative", region_string, search_terms[i]) # TODO: gives empty data frames for the groups neg/pos/unc/spe/hyd
  pie_plot_sides(counts_side_gk$neg_ct, "positive", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk$unc_ct, "uncharged", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk$spe_ct, "special cases", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk$hyd_ct, "hydrophobic", region_string, search_terms[i])
  ...
  pie_plot_sides(counts_side_gk_fl$pos_ct, "positive", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk_fl$neg_ct, "negative", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk_fl$unc_ct, "uncharged", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk_fl$spe_ct, "special cases", region_string, search_terms[i])
  pie_plot_sides(counts_side_gk_fl$hyd_ct, "hydrophobic", region_string, search_terms[i])
}
```
