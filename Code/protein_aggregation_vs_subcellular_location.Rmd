...
library(ggrepel)
library(tidyverse)
library(igraph) # Used for igraph
...
significance_subcellular(scores_protein)
get_krustal_test_results(scores_protein)
get_wilcox_test_table(scores_protein)
```
##Statistical tests and visualisation on secretory vs. non-secretory
```{r}
significance_secretory(scores_protein)
plot_ks_test(scores_protein)
```
