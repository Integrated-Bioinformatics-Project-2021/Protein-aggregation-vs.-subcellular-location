...
for (i in 1:dim(subcellular_locations)[1]) { #Deleting everything between {} and the "SUBCELLULAR LOCATION"
  ...
  string_location = gsub(" \\{.*\\}.", "", string_location)
  string_location = sub("S.*: ","", string_location)
...
#Auke's solution to use regex, had issues with some sampels I tried. i.e. proteins[5000:5005]
